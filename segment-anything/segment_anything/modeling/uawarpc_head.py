...
from .cma_utils import warp


# ── Inline BaseHead (from CMA models/heads/base.py) ──────────────────────────
class BaseHead(nn.Module):
    def __init__(self, num_classes, in_index, input_transform=None):
        super().__init__()
        self.input_transform = input_transform
        if isinstance(in_index, Iterable) and not isinstance(in_index, (str, bytes)):
    ...
    def _transform_inputs(self, inputs):
        if self.input_transform == 'resize_concat':
            ...
            upsampled = [
                F.interpolate(x, size=inputs[0].shape[2:],
...
# ── Inline ConvBNReLU (from CMA models/heads/modules.py) ─────────────────────
class ConvBNReLU(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 dilation=1, groups=1, padding=None, norm_layer=nn.BatchNorm2d,
                 activation_layer=nn.ReLU, bias='auto', depthwise_separable=False,
...
def _spatial_correlation_sample_pytorch(input1, input2, patch_size=9):
    """Pure PyTorch implementation of local spatial correlation (patch_size x patch_size shifts)."""
    B, C, H, W = input1.shape
    pad = patch_size // 2
    input2_padded = F.pad(input2, (pad, pad, pad, pad))
    corr_channels = []
    for dh in range(patch_size):
        for dw in range(patch_size):
            shifted = input2_padded[:, :, dh:dh + H, dw:dw + W]
            dot = (input1 * shifted).sum(dim=1, keepdim=True)
            corr_channels.append(dot)
    return torch.cat(corr_channels, dim=1)  # [B, patch_size^2, H, W]
...
class LocalFeatureCorrelationLayer(nn.Module):
    ...
    def forward(self, feature_source, feature_target):
        ...
        corr = _spatial_correlation_sample_pytorch(
            feature_target, feature_source, patch_size=self.patch_size)
