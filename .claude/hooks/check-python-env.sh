#!/usr/bin/env bash
# PreToolUse(Bash) hook — WeatherSAM 規則:Python/pip 必須走 sam_env。
...
if printf '%s' "$cmd" | grep -qE '(^|[;&|][[:space:]]*)(python3?|pip3?)([[:space:]]|$)'; then
  cat <<'EOF'
{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"WeatherSAM 規則:Python/pip 必須在 sam_env 中執行。請改用 conda run -n sam_env python ... 或 /home/rvl1421/miniconda3/envs/sam_env/bin/python(python -c 快速檢查與 py_compile 例外)。"}}
EOF
fi
exit 0
